...
- [x] [T50] リプレイ実装: 1m/15m リプレイと約定モデル（OHLC 内 fill、固定 bps 手数料/スリッページ）を実装。参照: `docs/design/backtester.md`。（依存: T21, T31, T40）
- [x] [T51] 集計: 勝率/PF/DD/平均保持時間のメトリクス集計と簡易サマリ出力を行う。（依存: T50）
- [x] [T50a] 実行導線整備: `config/` から Parquet パス・手数料/スリッページ・position_size/base_equity を読み込み、`run_backtest.py` などの CLI 入口と JSON 保存（BacktestResult/summary）を用意する。実データで即時実行できる手順を README に追記。（依存: T50, T51, T70, T60）
- [ ] [T52] BT 実行: 複数期間（例: 3 ヶ月/6 ヶ月/1 年）でバックテストを実行し、勝率・PF・最大 DD・平均保持時間・手数料影響を取得。結果を JSON または簡易レポートで残す。実行区間・手数料/スリッページ設定・現行パラメータも併記し、後続のレビュー材料にする。（依存: T50a, T70）
- [ ] [T53] パラメータ調整: VWAP 乖離/RSI/TP/SL/時間切れなど要件の調整可能項目を複数セット試し、指標が最良の候補を記録。必要なら `config/strategy.yaml` に反映案を準備し、BT 結果の共有・レビュー（Codex へ共有してすり合わせ）を前提にする。（依存: T52）
